...
class AutoScrollViewPager : ViewPager {
    ...
    public var interval = DEFAULT_INTERVAL.toLong()
...
//    var handler:Handler=Handler()
    ...
    fun startAutoScroll() {
        isAutoScroll = true
        sendScrollMessage((interval + scroller?.getDuration()!! / autoScrollFactor * swipeScrollFactor) as Long)
    }
    ...
    private class MyHandler(autoScrollViewPager: AutoScrollViewPager?) : Handler() {
        ...
        override fun handleMessage(msg: Message) {
            ...
            when (msg.what) {
                SCROLL_WHAT -> {
                    val pager: AutoScrollViewPager = autoScrollViewPager.get()!!
                    if (pager != null) {
